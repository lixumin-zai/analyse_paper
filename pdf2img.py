...
def get_dir_name(file_dir):
    ...
    dir_name = r"F:\data\test_paper\image"  # 获得地址的父链接
    # print(base_name, dir_name)
    # input()
    return dir_name, base_name

def mkdir(path, filename, type=''):
    path_file = path + os.sep + filename + type
    if os.path.exists(path_file):
        return mkdir(path, filename + '_副本', type)
    else:
        return path_file
...
def pdf2img(pdf_name):
    ...
    if pdf_name:
        if pdf_name[-4:].lower() == '.pdf':
            ...
            path = mkdir(dir_name, base_name[:-4])
            os.makedirs(path)
            ...
            for pg in range(0, pdf.pageCount):
                ...
                pm.writePNG(path + os.sep + base_name[:-4] + '_' + '{:0>3d}.{}'.format(pg+1, 'png'))  # 保存图片
            pdf.close()
            print('信息', '转化成功！', pdf_name)
        else:
            print('警告!', '文件类型错误,请打开pdf文件类型!')

if __name__ == '__main__':
    p = Pool(3)
    for root, ds, fs in os.walk(r'F:\data\done'):
        try:
            if fs:
                for f in tqdm.tqdm(fs):
                    try:
                        ...
                        # print(fullname)
                        # pdf2img(fullname)
                        p.apply_async(pdf2img, args=(fullname,))
